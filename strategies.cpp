...
/*** Variables globales ***/
ofstream csv;
vector<solution> population_glob;
...
void print_individu(solution s)
{
    ...
    cout << endl;
    cout << "  Volume consomme = " << s.volumeCons;
    cout.setf(ios::fixed);
    cout << " Fitness = " << s.eval  << endl;
}
...
void tri_tableau()
{
    ...
    // Tri
    cout << "\n Va trier" << nbobj << "  " << volumeSac << " " << min_volume;
...
void AG(AG_parameters paramAG, char* pop_init_file)
{
    int it=0;
    float taux_evol=0;
    float bestEval=0;
    cout << "\n RANI LDAKHEL l'AG avec population de taille " << population_act.size() << " parametres N et ratio" << param.N << " " << param.ratio << endl;
...
*/
    ...
    while(!check_arret(it,taux_evol,abs(bestEval-ref)))
    {
        ...
        cout << endl << endl << "\t POPULATION SELECTIONNEE A L'ITERATION " << it  << "de taille "  << population_int.size() << endl;
...
void initialeAleatoire(int nombrePopulation,int diversification)
{
    ...
    for(int i=0; i< nombrePopulation;i++)
    {
        ...
        population_glob.push_back(solution_actif);
    }
        ofstream trace;
        trace.open("population.txt");
        trace << "begin data\n";
        trace << nombrePopulation<<"\n";
        trace << nbobj <<endl;
        for (int ii=0;ii<nombrePopulation;ii++){
        //trace << "\nchromosome : "<< ii<<"\n";
        solution_actif=population_glob.at(ii);
         for(int j=0;j<nbobj;j++)
             {
...
void recup_AGparam(char* paramFile)
{
    ifstream f;
    f.open(paramFile);
    char row[512]="";
...
if (row[1] >= '0' && row[1]<='9' ){
            ...
            f.getline(row,512,'\n');
            if(strcmp(row,"end data\r")==0) endofdata=true;
            if (f.eof()) eof=true;
